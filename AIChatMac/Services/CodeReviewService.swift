...
final class CodeReviewService {
    ...
    private func reviewBatch(
        ...
        // Получаем текущие параметры AI из сервиса
        let aiParameters = await aiService.getCurrentAIParameters() //Value of type 'ChatService' has no member 'getCurrentAIParameters'
        ...
        if aiResponse.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines).isEmpty {
    ...
    private func createBatchReviewPrompt(
        ...
        // Динамические инструкции в зависимости от параметров AI
        ...
        case 0.0...0.3:
            styleInstructions = """
            ТВОЙ СТИЛЬ АНАЛИЗА (Температура \(String(format: "%.1f", temperature)) - Консервативный):
            - Будь методичным и систематичным
            - Фокусируйся на проверенных практиках
            - Избегай рискованных предположений
            - Следуй стандартным паттернам
            - Будь осторожен в оценках
            """
        case 0.4...0.7:
            styleInstructions = """
            ТВОЙ СТИЛЬ АНАЛИЗА (Температура \(String(format: "%.1f", temperature)) - Сбалансированный):
            - Будь внимательным и критичным
            - Ищи как очевидные, так и скрытые проблемы
            - Используй проверенные и инновационные подходы
            - Предлагай разумные улучшения
            - Оценивай риски и выгоды
            """
        case 0.8...1.2:
            styleInstructions = """
            ТВОЙ СТИЛЬ АНАЛИЗА (Температура \(String(format: "%.1f", temperature)) - Креативный):
            - Дай волю своей интуиции и опыту
            - Ищи нестандартные решения и подходы
            - Не бойся предлагать инновационные идеи
            - Если видишь паттерны - отмечай их
            - Будь более экспрессивным в оценках
            """
        default:
            styleInstructions = """
            ТВОЙ СТИЛЬ АНАЛИЗА (Температура \(String(format: "%.1f", temperature)) - Экспериментальный):
            - Будь максимально креативным
            - Предлагай необычные решения
            - Ищи скрытые возможности
            - Не ограничивай себя стандартами
            - Экспериментируй с подходами
            """
        }
        ...
        case 50...500:
            detailInstructions = """
            ⚡ ОГРАНИЧЕНИЕ ТОКЕНОВ (\(maxTokens)): Будь максимально кратким!
            - Фокусируйся только на критических проблемах
            - Используй короткие, но точные описания
            - Опускай детали, оставляй только суть
            - Приоритизируй проблемы по важности
            """
        case 501...2000:
            detailInstructions = """
            ⚡ ОГРАНИЧЕНИЕ ТОКЕНОВ (\(maxTokens)): Будь лаконичным!
            - Описывай проблемы кратко, но понятно
            - Давай основные предложения по исправлению
            - Избегай излишних деталей
            - Фокусируйся на важных аспектах
            """
        case 2001...4000:
            detailInstructions = """
            ⚡ ОГРАНИЧЕНИЕ ТОКЕНОВ (\(maxTokens)): Стандартная детальность
            - Давай подробные описания проблем
            - Предлагай конкретные решения с примерами
            - Объясняй причины и последствия
            - Включай рекомендации по улучшению
            """
        default:
            detailInstructions = """
            ⚡ ОГРАНИЧЕНИЕ ТОКЕНОВ (\(maxTokens)): Максимальная детальность
            - Давай исчерпывающие описания
            - Предлагай детальные решения с кодом
            - Объясняй архитектурные решения
            - Включай альтернативные подходы
        ...
        🔍 ЧТО ИСКАТЬ:
        - Проблемы безопасности (SQL injection, XSS, etc.)
        ...
        - Плохое именование переменных/функций
        
        💡 ДЛЯ КАЖДОЙ ПРОБЛЕМЫ:
        - Объясни ПОЧЕМУ это проблема
        - Предложи КОНКРЕТНОЕ решение
        - Если возможно - покажи код исправления
        
        📝 ФОРМАТ ОТВЕТА:
        ОЦЕНКА: [1-100] (где 100 = идеальный код)
        
        ПРОБЛЕМЫ:
        - [Тип] [Файл:строка] - [Описание проблемы] - [Конкретное предложение исправления]
        
        ПРЕДУПРЕЖДЕНИЯ:
        - [Тип] [Файл:строка] - [Описание] - [Рекомендация]
        
        ПРЕДЛОЖЕНИЯ:
        - [Тип] [Файл:строка] - [Описание улучшения] - [Код или детальное предложение]
        
        КОНТЕКСТ:
        [Твои наблюдения, паттерны, общие выводы для следующих батчей]
        
        ⚡ ВАЖНО: Адаптируй детальность ответа под доступное количество токенов!
        """
